...
public class JWEffect : MonoBehaviour {
	...
	IEnumerator Blink(){
		...
		while (true) {
			...
			if (blink){
				if (alphaUp){
					float currMaxAlpha = AddColorReturnMaxAlpha(new Color(0,0,0, blinkSpeed*Time.deltaTime));
	...
	IEnumerator Blink_Grey(){
		bool greyUp = true;
		float cumulativeGrey = 0;
		while (true) {
			yield return new WaitForSeconds(Time.deltaTime);
			if (blinkGrey){
				float offset = blinkSpeed*Time.deltaTime;
				if (greyUp){
